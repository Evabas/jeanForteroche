<?php
session_start();
?>
<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <link rel="stylesheet" href="jf.css" />
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous" />
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.2/css/all.css" integrity="sha384-oS3vJWv+0UjzBfQzYUhtDYW+Pj2yciDJxpsK1OYPAYjqT085Qq/1cq5FLXAZQ7Ay" crossorigin="anonymous">

    <title>Commentaires</title>
</head>

<body>
<header>

<?php include("menu.php"); ?>
    
</header>

<section class="marge">
Membre connecté(e) :  
<?php  if (isset($_SESSION['pseudo'])){echo $_SESSION['pseudo'];}?> <br>
<?php echo '<a href="./logout.php">Déconnection</a>';?><br>
        
        <p><a href="chapitres.php">Retour aux chapitres</a></p>

<?php
// Connexion à la base de données
try
{
	$bdd = new PDO('mysql:host=localhost;dbname=blog;charset=utf8', 'root', '');
}
catch(Exception $e)
{
        die('Erreur : '.$e->getMessage());
}

// Récupération du chapitre
if(isset($_GET['chapitre'])){
$chapitre = $_GET['chapitre'];
$req = $bdd->prepare('SELECT id, titre, contenu FROM chapitres WHERE id = ?');
$req->execute(array($chapitre));
$donnees = $req->fetch();
?>

<div class="container">
        <div class="row">
            <div class="col-md-9 offset-md-1">
    <h2>
        <?php echo htmlspecialchars($donnees['titre']); ?>
    </h2>
    
    <p>
    <?php
    echo nl2br($donnees['contenu']);
    ?>
    </p>

<h3>Commentaires</h3>

<?php

$req->closeCursor(); 
}
// Récupération des commentaires
if (isset($_GET['chapitre'])){
  
$req = $bdd->prepare('SELECT pseudo, commentaire FROM commentaires WHERE id_chapitre = ?');
$req->execute(array($_GET['chapitre']));
          
while ($resultat = $req->fetch()){
?>
<p><strong><?php echo htmlspecialchars($resultat['pseudo']); ?></strong></p>
<p><?php echo nl2br(htmlspecialchars($resultat['commentaire'])); ?></p>
<?php
}
$req->closeCursor();
}
?>
<br>
</p>
    <form action="commentaires_post.php?chapitre=<?php echo $_GET['chapitre']; ?>" method="post">
        <p>
        <label for="pseudo">Pseudo</label> : <input type="text" name="pseudo" id="pseudo" /><br />
        <label for="commentaire">Commentaire</label> :  <input type="text" name="commentaire" id="commentaire" /><br />

        <button type="submit" class="btn btn-primary mb-2">Envoyer</button>
    </form>
</p>
</div>
    </div>
        </div>
<footer id="pied_de_page">

</footer>
<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
</body>

</html>


